...
export default function GameScreen() {
  ...
  useEffect(() => {
    const loadMilestonesAndStats = async () => {
  ...
  function handleMenuPress() {
    console.log('[Game] Menu button pressed - showing iOS ActionSheet');
    ActionSheetIOS.showActionSheetWithOptions(
      {
        options: ['Cancel', 'Start New Game', 'Back to Home Screen'],
        cancelButtonIndex: 0,
      ...
      (buttonIndex) => {
        if (buttonIndex === 1) {
          console.log('[Game] New game selected from ActionSheet');
          setConfirmNewGameVisible(true);
        } else if (buttonIndex === 2) {
          console.log('[Game] Back to home selected from ActionSheet');
          router.replace('/');
        }
      }
    );
